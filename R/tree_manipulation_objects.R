...
grow_projection_tree <- function(tree, x, node_min_size, theta = NULL) {
  ...
  while(bad_trees) {
    ...
    node_var_split <- sample_split_var(x_var = x[,node_var],node_min_size = node_min_size)
    ...
    i <- max(vapply(new_tree, "[[", numeric(1), "node_number")) + 1
...
grow_tree <- function(tree, x, node_min_size, rotation = TRUE, theta = NULL){
  ...
  while(bad_trees) {
    ...
    if(is.numeric(x[, node_var])) {
      ...
      node_var_split <- sample_split_var(x_var = x[current_node$observations_index,node_var],node_min_size = node_min_size)
...
swap_tree_verb <- function(tree, x, node_min_size) {
  ...
  while(bad_trees) {
    ...
    for(i in seq_along(list_nodes)) {
      ...
      # Verifying the type of the current node
      if(is.list(current_node_aux$node_var)) {
...
# Sample uniform split_var_rule
sample_split_var <- function(x_var, node_min_size){


  # Having the range of values
  sort_x <- sort(x_var)
  available_splits <- sort_x[(node_min_size+1):(length(sort_x)-node_min_size)]

  # Return the random sampled value
  return(stats::runif(n = 1,min = min(available_splits), max = max(available_splits)))
